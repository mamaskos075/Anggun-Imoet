...
document.getElementById('mitraForm').addEventListener('submit', function(event) {
    ...
    // Bersihkan formulir setelah pengiriman (opsional)
    this.reset();
});

// Pastikan tautan "Daftar Jadi Mitra" di header mengarah ke halaman ini
document.addEventListener('DOMContentLoaded', function() {
    const navLinks = document.querySelectorAll('.main-nav .nav a');
    navLinks.forEach(link => {
        if (link.textContent.includes('Daftar Jadi Mitra')) {
            link.href = 'daftar-mitra.html';
        }
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(event) {
        if (!filterOptions.contains(event.target) && !toggleFilterButton.contains(event.target)) {
            filterOptions.classList.remove('show');
        }
    });

    // Apply filter functionality (currently logs to console)
    applyFilterButton.addEventListener('click', function() {
        ...
        document.querySelectorAll('input[name="wilayah"]:checked').forEach(checkbox => {
            selectedFilters.wilayah.push(checkbox.value);
        });

        // Get selected harga
        document.querySelectorAll('input[name="harga"]:checked').forEach(checkbox => {
            selectedFilters.harga.push(checkbox.value);
        });

        // Get selected fasilitas
        document.querySelectorAll('input[name="fasilitas"]:checked').forEach(checkbox => {
            selectedFilters.fasilitas.push(checkbox.value);
        });

        console.log("Filters Applied:", selectedFilters);
        // Di sini Anda bisa menambahkan logika untuk memfilter daftar kos yang ditampilkan
        // Misalnya, Anda bisa memanggil fungsi untuk memuat ulang daftar kos berdasarkan filter yang dipilih.

        // Menutup filter setelah menerapkan
        filterOptions.classList.remove('show');
    });
});
